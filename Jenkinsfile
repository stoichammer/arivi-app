        ...
        script {
          if ((env.BRANCH_NAME).startsWith("release")) {   
            ...
            dir(path: 'allpay-proxy') {
                    ...
                    sh 'zip allpay-proxy_"$(basename $(git symbolic-ref HEAD))"_ubuntu1804.zip aaproxy allpay-proxy-config.yaml README.md Checksum_SHA256 LICENSE LICENSE-AGPL LICENSE-OpenBSV'
                  }
            ...
            dir(path: 'allpay-proxy') {
                    ...
                    sh 'rm -f /tmp/proxy-ubuntu2004.cid'
                    sh 'sha256sum ./aaproxy > Checksum_SHA256'
                    sh 'zip allpay-proxy_"$(basename $(git symbolic-ref HEAD))"_ubuntu2004.zip aaproxy allpay-proxy-config.yaml README.md Checksum_SHA256 LICENSE LICENSE-AGPL LICENSE-OpenBSV'
                  }
            ...
            dir(path: 'allpay-proxy') {
                    ...
                    sh 'rm -f /tmp/proxy-archlinux.cid'
                    sh 'sha256sum ./aaproxy > Checksum_SHA256'
                    sh 'zip allpay-proxy_"$(basename $(git symbolic-ref HEAD))"_archlinux.zip aaproxy allpay-proxy-config.yaml README.md Checksum_SHA256 LICENSE LICENSE-AGPL LICENSE-OpenBSV'
                  }
