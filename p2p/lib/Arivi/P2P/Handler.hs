...
processRequest connHandle p2pMessage peerNodeId = do
  ...
  let p2pResponse = generateP2PMessage (uuid p2pMessage)
                                       (messageType p2pMessage)
                                       responseMsg
  res <- LE.try $ send connHandle (serialise p2pResponse)
  case res of
    Left (e :: AriviNetworkException) ->
      logWithNodeId peerNodeId "network send failed while sending response"
...
processIncomingMessage connHandle peerDetailsTVar msg = do
  ...
  case p2pMessageOrFail of
    Left _ -> logWithNodeId peerNodeId "Peer sent malformed msg"
      >> return (Left DeserialiseFailureP2P)
    Right p2pMessage -> do
      peerDetails <- liftIO $ atomically $ readTVar peerDetailsTVar
      case uuid p2pMessage of
        Just uid -> case peerDetails ^. uuidMap . at uid of
...
readIncomingMessage connHandle peerDetailsTVar = do
  ...
  case msgOrFail of
    Left (e :: AriviNetworkException) -> void $ logWithNodeId
      peerNodeId
      ("network recv failed from readIncomingMessage" ++ displayException e)
    Right msg -> do
...
newIncomingConnectionHandler nc transportType connHandle = do
  ...
  case HM.lookup (nc ^. nodeId) nodeIdPeerMap of
    Nothing -> do
      peerDetailsTVar <- liftIO $ atomically $ mkPeer nc
                                                      transportType
                                                      (Connected connHandle)
      liftIO $ atomically $ addNewPeer (nc ^. nodeId)
                                       peerDetailsTVar
                                       nodeIdMapTVar
    Just peerDetailsTVar -> liftIO $ atomically $ updatePeer
      transportType
      (Connected connHandle)
      peerDetailsTVar
  nodeIdPeerMap' <- liftIO $ atomically $ readTVar nodeIdMapTVar
