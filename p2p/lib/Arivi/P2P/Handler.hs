...
processIncomingMessage connHandle peerDetailsTVar msg = do
  ...
  case p2pMessageOrFail of
    Left _ ->
      logWithNodeId peerNodeId "Peer sent malformed msg" >>
...
newIncomingConnectionHandler nc transportType connHandle = do
  ...
  case HM.lookup (nc ^. nodeId) nodeIdPeerMap of
    Nothing -> do
      peerDetailsTVar <-
        liftIO $ atomically $ mkPeer nc transportType (Connected connHandle)
      liftIO $ atomically $
        addNewPeer (nc ^. nodeId) peerDetailsTVar nodeIdMapTVar
    Just peerDetailsTVar ->
      liftIO $ atomically $
      updatePeer transportType (Connected connHandle) peerDetailsTVar
  nodeIdPeerMap' <- liftIO $ atomically $ readTVar nodeIdMapTVar
