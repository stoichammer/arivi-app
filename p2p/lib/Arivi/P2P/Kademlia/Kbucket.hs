...
getDefaultNodeId
  :: forall m . (HasKbucket m, MonadIO m) => ExceptT AriviP2PException m NodeId
getDefaultNodeId = do
  kbucket' <- lift getKb
...
removePeer peerR = do
  ...
  let kb         = getKbucket kbb'
      ...
      pl2        = fmap (nodeID) pl
  if peerR `elem` pl2
    then liftIO $ atomically $ H.insert
      (L.deleteBy (\p1 p2 -> (nodeID p1) == (nodeID p2)) fp pl)
      kbDistance
      kb
...
getPeerByNodeId nid = do
  ...
  if null t then throwError KademliaPeerDoesNotExist else return $ head t
