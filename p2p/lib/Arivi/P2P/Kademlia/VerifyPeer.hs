...
import           Arivi.P2P.Kademlia.Utils       ( addListOfList
                                                , count'
                                                )
...
isVerified
  :: (HasKbucket m, MonadIO m) => Peer -> ExceptT AriviP2PException m NodeStatus
isVerified peer = do
  kb <- lift getKb
...
getVerifiedPeers peerR k = do
  ...
  vpl <- filterM
    ...
    (addListOfList fll)
  if length vpl > k then return (fst $ splitAt k vpl) else return vpl
...
filterPeer nid rnid peerL = result
 where
  result = filter
    (\x -> rXor >= getXorDistance (C.unpack $ BS.encode nid)
...
isVNRESPValid peerL peerR = do
  ...
  let firstCheck | Peer dnid undefined `elem` temp = True
...
responseHandler resp peerR peerT = case resp of
  Right pl -> do
    ...
    let kbm2   = nodeStatusTable kb
        kbtemp = H.stream kbm2
    kvList <- liftIO $ atomically $ toList kbtemp
    $(logDebug) $ T.append (T.pack "NodeStatusTable after adding : ")
                           (T.pack (show kvList))
