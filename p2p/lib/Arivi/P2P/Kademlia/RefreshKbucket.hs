...
issuePing rpeer = do
  ...
  case resp of
    Left e -> do
      ...
      return False
    Right (KademliaResponse payload) -> case messageBody (message payload) of
      PONG _ _ -> return True
      _        -> return False

deleteIfExist :: Peer -> [Peer] -> [Peer]
deleteIfExist peerR pl = if peerR `elem` pl
  then L.deleteBy (\p1 p2 -> (nodeID p1) == (nodeID p2))
