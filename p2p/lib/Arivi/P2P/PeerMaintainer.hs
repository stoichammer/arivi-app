...
isFilledHelper minimumNodes l  = not
  <$> (fmap (any (< minimumNodes)) <$> mapM (fmap Set.size <$> readTVarIO)) l
...
addPeerFromKademlia
  :: (HasNodeEndpoint m, MonadIO m) => [KademliaTypes.Peer] -> m [NodeId]
addPeerFromKademlia = mapM
  (\peer -> do
...
addPeerFromKademliaHelper
  :: (MonadIO m) => KademliaTypes.Peer -> TVar NodeIdPeerMap -> m NodeId
addPeerFromKademliaHelper peerFromKademlia nodeIdPeerMapTVar =
  liftIO $ atomically
    (do
      nodeIdPeerMap <- readTVar nodeIdPeerMapTVar
      ...
      case mapEntry of
        Nothing -> do
          defaultPeer <-
            (& networkConfig .~ NetworkConfig {..}) <$> defaultPeerDetails
