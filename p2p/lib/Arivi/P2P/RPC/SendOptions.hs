...
sendOptionsToPeer recievingPeerNodeId optionsMsg = do
  res <- runExceptT
    $ issueRequest recievingPeerNodeId (OptionRequest optionsMsg)
  case res of
...
updateResourcers
  :: (MonadReader env m, HasRpc env r msg, MonadIO m) => NodeId -> [r] -> m ()
...
updateResourcers'
  :: (Ord r, Hashable r, MonadIO m) => NodeId -> Resourcers r -> r -> m ()
