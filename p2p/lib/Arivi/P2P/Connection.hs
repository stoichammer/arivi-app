...
createConnection peerDetailsTVar _ transportType = do
  ...
  connHandleEither <- case checkConnection peerDetails transportType of
    Connected connHandle -> return $ Right connHandle
    NotConnected ->
      networkToP2PException
        <$> openConnectionToPeer (peerDetails ^. networkConfig) transportType
  liftIO $ atomically $ putTMVar (peerDetails ^. connectionLock) lock
  case connHandleEither of
    Right c -> do
      liftIO $ atomically $ updatePeer transportType
                                       (Connected c)
                                       peerDetailsTVar
