...
sendAndReceive peerDetailsTVar messageType connHandle msg = do
  ...
  res <- networkToP2PException
    <$> LE.try (send connHandle (serialise p2pMessage))
  case res of
    Left e -> do
    ...
    Right () -> do
      winner <- liftIO
        $ Async.race (threadDelay 30000000) (takeMVar mvar :: IO P2PMessage)
      case winner of
        Left _ ->
          $(logDebug) "response timed out" >> return (Left SendMessageTimeout)
...
issueSend peerNodeId uuid req = do
  ...
  connHandle    <- ExceptT $ getConnectionHandle
    peerNodeId
    nodeIdMapTVar
    (getTransportType $ msgType (Proxy :: Proxy (Request t i)))
  ExceptT $ sendWithoutUUID peerNodeId
...
issueRequest peerNodeId req = do
  ...
  connHandle    <- ExceptT $ getConnectionHandle
    ...
    nodeIdMapTVar
    (getTransportType $ msgType (Proxy :: Proxy (Request t i)))
  peerDetailsTVar <- maybe (throwError PeerNotFound)
                           return
                           (nodeIdPeerMap ^. at peerNodeId)
  resp <- ExceptT $ sendAndReceive peerDetailsTVar
                                   (msgType (Proxy :: Proxy (Request t i)))
                                   connHandle
                                   (serialise req)
