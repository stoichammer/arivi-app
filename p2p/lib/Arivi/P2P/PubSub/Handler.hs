...
subscribeHandler nid (PubSubRequest (PubSubPayload (t, subTimer))) = do
  ...
  if success then return (PubSubResponse Ok) else return (PubSubResponse Error)


handleTopic :: (HasP2PEnv env m r t rmsg pmsg) => NodeId -> pmsg -> m Status
handleTopic _nid msg = do
