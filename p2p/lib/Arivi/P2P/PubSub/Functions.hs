...
publishTopic messageTopic publishMessage = do
  ...
  currWatcherSortedList <- case currWatcherListTvarMaybe of
  ...
  currNotifierSortedList <- case currNotifierListTvarMaybe of
    Nothing                -> return $ toSortedList []
...
notifyTopic mTopic mTopicMessage = do
  ...
  case listOfWatchersForTopic of
    ...
    Just currWListTVar -> do
      ...
      case currTuple of
        Nothing ->
          sendMultiplePubSubMessage watcherNodeIdList notifyMessageByteString
        Just valueTuple -> do
        ...
        (case responseMessage of
          Response mresponseCode mTimer -> do
            ...
            case mresponseCode of
              Ok -> do
                ...
                case currNotifierListTvar of
                  Nothing -> do
                    ...
                    let updatedMap =
                          HM.insert mTopic (newNotifTvar, 5) notifierMap
                    writeTVar notifierTableTVar updatedMap
                    let errorMessage =
                          Response {responseCode = Ok, messageTimer = 0}
                    return $ serialise errorMessage
                  Just entry -> do
              ...
              _ -> do
                let errorMessage = Response
                      { responseCode = Error {errCode = Unknown}
                      , messageTimer = 0
                      }
                return $ serialise errorMessage
          _ -> do
            let errorMessage = Response
                  { responseCode = Error {errCode = InvalidResponseError}
                  , messageTimer = 0
                  }
            return $ serialise errorMessage
        )
...
updateTransientResourceToPeerMap resID resHandler nodeID = do
  ...
  liftIO $ atomically
    (do
      transientResourceToPeerMap <- readTVar transientResourceToPeerMapTVar
      let currentEntry = HM.lookup resID transientResourceToPeerMap
      case currentEntry of
        Nothing -> do
          newNodeTVar <- newTVar [nodeID]
          let modifiedMap = HM.insert resID
...
maintainWatchersHelper watcherMap (topic : topicIdList) = do
  ...
  case currListTvarMaybe of
    ...
    Just currListTvar -> do
      ...
      liftIO $ atomically
        (do
          currSortedList <- readTVar currListTvar
...
maintainNotifiersHelper notifierMap (currTopic : topicList) = do
  ...
  case currNotifierListTvar of
    ...
    Just mapValue -> do
      currTime      <- liftIO getCurrentTime
      expiredNotifs <- liftIO $ atomically
        (do
          currSortedList <- readTVar $ fst mapValue
          let currNotiferList     = fromSortedList currSortedList
          ...
          writeTVar (fst $ fromJust currNotifierListTvar)
...
maintainMinimumCountNotifier mTopic = do
  ...
  case currNotifierListTuple of
    ...
    Just entry -> do
      ...
      let currNotiferList      = fromSortedList currSortedList
      Control.Monad.when (length currNotiferList - minNumberOfNotifiers < 0)
        $ do
...
pubsubHandler incomingRequest = do
  ...
  case deserialiseCheck of
    Left _ -> do
      ...
      return $ serialise errorMessage
    Right (incomingMessage :: MessageTypePubSub) -> case incomingMessage of
      Notify recvNodeId mTopicId mTopicMessage -> notifyMessageHandler
        recvNodeId
        mTopicId
        mTopicMessage
        messageMapTVar
        notifierTableTVar
        topicHandlerTVar
      Publish recvNodeId mTopicId mTopicMessage -> verifyIncomingMessage
        recvNodeId
        mTopicId
        mTopicMessage
        messageMapTVar
...
notifyMessageHandler
  ...
  -> m P2PPayload
notifyMessageHandler recvNodeId mTopicId mTopicMessage messageMapTVar notifierTableTVar topicHandlerTVar
  = do
    ...
    case entry of
      ...
      Just notifierListTuple -> do
        ...
        if recvNodeId `elemOrd` listOfNodeIds
          then verifyIncomingMessage recvNodeId
                                     mTopicId
                                     mTopicMessage
                                     messageMapTVar
                                     topicHandlerTVar
            ...
            let errMessage = Response
                  { responseCode = Error {errCode = InvalidNotifierError}
                  , messageTimer = 0
                  }
            return $ serialise errMessage
...
verifyIncomingMessage recvNodeId mTopicId mTopicMessage messageMapTVar topicHandlerTVar
  = do
    returnMessage <- liftIO $ atomically
      (do
        messageMap <- readTVar messageMapTVar
        ...
        case entryInMessageMap of
          Nothing -> do
            ...
            case entryTopicMap of
              Nothing -> return
                (Error {errCode = TopicHandlerNotRegistered}, mTopicMessage)
    ...
    case checkReturn of
      ...
      Error code -> do
        let errorMessage =
              Response {responseCode = Error {errCode = code}, messageTimer = 0}
        return $ serialise errorMessage
...
subscribeMessageHandler mNodeId mTopic mTime watcherTableTVar = do
  ...
  case entry of
    ...
    Just watcherTVar -> liftIO $ atomically
      (do
        watcherSortedList <- readTVar watcherTVar
...
responseMessageHandler rCode = case rCode of
  Ok -> do
    let resp              = rCode
